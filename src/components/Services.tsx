...
export default function Services() {
  return (
    <section className="py-24 px-6 md:px-24 bg-[#121212] relative">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h2 className="text-4xl md:text-6xl font-bold text-white tracking-tighter uppercase mb-4">
            Technical <span className="text-white/40">Expertise</span>
          </h2>
          <p className="text-white/50 font-mono text-sm tracking-[0.3em] uppercase">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="glass p-10 rounded-3xl relative overflow-hidden group border-white/5"
            >
              {/* Hover Gradient Effect */}
              <div className={`absolute inset-0 bg-gradient-to-br ${service.color} opacity-0 group-hover:opacity-100 transition-opacity duration-500`} />
              
              <div className="relative z-10">
                <div className="w-16 h-16 rounded-2xl bg-white/5 flex items-center justify-center mb-8 text-white group-hover:scale-110 transition-transform duration-500 border border-white/10">
                  {service.icon}
                </div>
                ...
                <div className="mt-8 flex items-center text-white/30 group-hover:text-white/80 transition-colors duration-300">
